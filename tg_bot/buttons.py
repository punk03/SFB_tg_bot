...
all_posts = InlineKeyboardButton(
    "📰 Стена сообщества", url="https://t.me/stroy_forum33"
)
...
# Генератор клавиатуры с кнопкой "Вернуться к категориям мастеров" сверху и снизу
def generator_with_masters_button(buttons_list=None, row_width=1, force_single_column=True, preserve_emoji=True, sort_alphabetically=True):
    """
    Создает клавиатуру с кнопкой "Вернуться к категориям мастеров" вверху и внизу
    :param buttons_list: список названий кнопок (если None, то только кнопки навигации)
    :param row_width: количество кнопок в ряду (по умолчанию 1)
    :param force_single_column: принудительно размещать кнопки в одну колонку (по умолчанию True)
    :param preserve_emoji: не добавлять эмодзи к кнопкам, если они уже есть в названии (по умолчанию True)
    :param sort_alphabetically: сортировать ли кнопки по алфавиту
    ...
    # Кнопка "Вернуться к категориям мастеров" вверху
    kb.row(KeyboardButton('◀️ Вернуться к категориям мастеров'))
    ...
    if buttons_list:
        ...
        # Для списка мастеров - по одной кнопке в ряд
        for button in buttons_list:
            # Удаляем эмодзи из названия категории
            button_name = button
            for emoji in emoji_list:
    ...
    # Кнопка "Вернуться к категориям мастеров" внизу
    kb.row(KeyboardButton('◀️ Вернуться к категориям мастеров'))
...
def master_works_keyboard():
    """
    Создает клавиатуру только с кнопками навигации по иерархии для просмотра работ мастера
    :return: ReplyKeyboardMarkup
    """
    kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    # Добавляем кнопку возврата к анкете мастера
    kb.add(KeyboardButton('◀️ Вернуться к анкете мастера'))
